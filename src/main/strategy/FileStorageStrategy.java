...
public class FileStorageStrategy implements StorageStrategy {
    ...
    private void init() {
        // создаем новый массив корзин начального размера
        table = new FileBucket[DEFAULT_INITIAL_CAPACITY];
        for (int i = 0; i < table.length; i++) {
            //заполняем этот массив новыми объектами-корзинами
            table[i] = new FileBucket();
        }
    }

    static int indexFor(int hash, int length) {
        // возвращает логическое(бинарное???) и длины и хэша
        return hash & (length - 1);
    }

    final Entry getEntry(Long key) {
        // если наш массив пуст, то возвращаем нуль
        if (size == 0) {
            return null;
        }
        // иначе перебираем записи в массиве и сравниваем с ключом
        ...
        //если такого ключа нет возвращаем нал
        return null;
    }
